<template>
  <label :for="props.name" class="relative group">
    ...
    <textarea v-model="inputValue" :name="props.name" :readonly="props.readonly" :disabled="props.disabled" class="block border border-dark-gray rounded-lg text-current px-4 py-3 focus:border-yellow focus:outline-none w-full" @update:modelValue="emitValue($event)"/>
