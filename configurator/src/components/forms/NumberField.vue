<template>
  <label :for="props.name" class="relative group w-full">
    ...
    <input type="number" v-model="inputValue" :name="props.name" class="border bg-white read-only:text-dark-gray border-dark-gray rounded-lg text-black px-4 py-3 w-full focus:border-yellow focus:outline-none" :readonly="readonly" :disabled="disabled" @update:modelValue="emitValue($event)"/>
