<template>
  <label :for="name" class="h-full relative group">
    ...
    <input type="password" v-model="inputValue" :name="name" :readonly="readonly" :disabled="disabled" class="border border-dark-gray rounded-lg text-current px-4 py-3 focus:border-yellow focus:outline-none w-full" :class="error ? 'border-red' : ''" @update:modelValue="emitValue($event)"/>
